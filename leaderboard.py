...
class Leaderboard(object):
    ...
    def draw(self, win):
        ...
        for i, score in enumerate(scores):
            ...
            win.blit(rank, (self.x + 5 , self.y + i*self.HEIGHT_ENTRY + self.HEIGHT_ENTRY/2 - rank.get_height()/2))

            name = self.name_font.render(score[0], 1, (0,0,0))
            win.blit(name, (self.x - name.get_width()/2 + self.WIDTH/2, self.y + i*self.HEIGHT_ENTRY + 10))

            score = self.score_font.render("Score: " + str(score[1]), 1, (0, 0, 0))
            win.blit(score, (self.x +10, self.y + i * self.HEIGHT_ENTRY + 40))
